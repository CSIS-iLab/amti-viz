var basemap = L.tileLayer("https://api.mapbox.com/styles/v1/ilabmedia/cj84s9bet10f52ro2lrna50yg/tiles/256/{z}/{x}/{y}@2x?access_token=pk.eyJ1IjoiaWxhYm1lZGlhIiwiYSI6ImNpbHYycXZ2bTAxajZ1c2tzdWU1b3gydnYifQ.AHxl8pPZsjsqoz95-604nw", {});
var map = L.map("map", {
    center: [
        30.27,
        125.05
...
const mapSource = new carto.source.SQL(`SELECT * FROM east_china_sea_oil_rig_locations_map ORDER BY cartodb_id DESC`);
const chinaContinentalShelfSource = new carto.source.SQL(`SELECT * FROM china_continental_shelf_line`);
const japanContinentalShelfSource = new carto.source.SQL(`SELECT * FROM japan_continental_shelf_line`);
const joinDevelopmentZoneSource = new carto.source.SQL(`SELECT * FROM joint_development_zone_line`);
...
const mapLayerChinaShelf = new carto.layer.Layer(chinaContinentalShelfSource, chinaContinentalShelfStyle, {
    featureOverColumns: [
        "name"
    ]
});
const mapLayerJapanShelf = new carto.layer.Layer(japanContinentalShelfSource, japanContinentalShelfStyle, {
    featureOverColumns: [
        "name"
    ]
});
const mapLayerJoinDevelopmentZone = new carto.layer.Layer(joinDevelopmentZoneSource, joinDevelopmentZoneStyle, {
    featureOverColumns: [
        "name"
    ]
});
